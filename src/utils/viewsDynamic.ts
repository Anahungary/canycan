// utils/viewsDynamic.ts - SISTEMA DE VISTAS DINÁMICAS COMPLETO CON NETLIFY
...
interface TrendingArticle {
  ...
  recent_views: number;
  total_views: number;
  reading_time?: number; // ✅ AGREGADO PARA COMPATIBILIDAD
}
    ...
    try {
      ...
      if (trendingFromDB && trendingFromDB.length > 0) {
        ...
        return trendingFromDB.map((article: TrendingArticle) => ({
          ...
          reading_time: article.reading_time || 5 // ✅ AGREGADO
        }));
      } else {
      ...
      return fallbackArticles.slice(0, limit).map((article: FallbackArticle) => ({
        ...
        reading_time: article.readingTime || 5 // ✅ AGREGADO
...
export function trackArticleView(slug: string): void {
  ...
  // URL para Netlify Functions vs API routes
  const apiUrl = window.location.hostname === 'localhost' 
    ? '/api/article-view'  // En desarrollo usar API routes
    : '/.netlify/functions/article-view';  // En producción usar Netlify Functions
...
// 🎯 SCRIPT PARA INCLUIR EN PÁGINAS DE ARTÍCULOS - NETLIFY COMPATIBLE
...
<script>
  ...
  setTimeout(() => {
    ...
    if (slug && !sessionStorage.getItem('viewed_' + slug)) {
      ...
      // URL para Netlify Functions vs desarrollo
      const apiUrl = window.location.hostname === 'localhost' 
        ? '/api/article-view'  // En desarrollo
        : '/.netlify/functions/article-view';  // En producción (Netlify)
